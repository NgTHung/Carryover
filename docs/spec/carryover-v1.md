...
Use Expo Router for navigation, stable NativeWind with compatible Tailwind CSS and Tailwind Variants for styling, and Reanimated directly for animation. Keep Hermes bundled with Expo and React Native. Moti is deferred. Jest and React Native Testing Library run locally and in Linux CI. Native build checks remain on code pushes, with the widget excluded by default, and candidate releases are checked manually on the iPhone. Automated iOS end-to-end testing is deferred. See [App stack and testing](../app-stack-and-testing.md) for the implementation contract.
