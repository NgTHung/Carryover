...
SQLite CHECK constraints enforce integer storage, sign, and the same safe-integer ceiling for every amount column. Existing installations receive the ceiling through migration triggers because SQLite cannot toggle foreign-key enforcement during Drizzle's migration transaction. They protect writes that bypass Zod or ORM mapping. The data layer also checks facts that need stored rows, such as whether a category is an active leaf. Pure functions own split arithmetic and budget arithmetic. Runtime shape validation does not replace those checks or their tests.
