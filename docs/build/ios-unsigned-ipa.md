...
The first CI runs with Ccache enabled produced IPAs but reported zero cache reads and writes. The build script now generates compiler wrappers with absolute Ccache and Clang paths and explicit cache configuration. They retain the intended cache directory even without the workflow environment. Xcode receives these wrappers through CC and CXX; linking uses Clang directly. Clang explicit modules are disabled because Xcode does not recognize a wrapper as its compiler. Swift's module setting stays unchanged. The build script also participates in the cache key. A populated cache and a subsequent build with cache hits still need CI evidence.
