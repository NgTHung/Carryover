...
Add two development-only feedback loops. A browser preview runs on the local machine for routine screen and style work. A separately packaged unsigned iOS development build connects to the local Expo server for device-accurate Fast Refresh. The release workflow and widget opt-in remain unchanged.
...
- [ ] The browser path supports the current SQLite migration flow with the required Metro WASM and isolation configuration.
