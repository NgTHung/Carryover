---
id: "WIDGET-002"
title: "The widget reading the snapshot"
status: To Do
priority: "Medium"
type: "Epic"
milestone: "0.7.0"
depends_on: ["WIDGET-001", "app:BUDGET-002", "app:DATA-008"]
risk: "High"
impact: "The last stage, and the one with a known unresolved failure. If the extension still renders stale code from a clean device, the problem is in the install layer and the paid membership will not fix it."
tags: ["widget", "ios", "epic"]
last_updated: 2026-09-13
---

## Summary

Stage 6 completes widget functionality before the UI overhaul. WIDGET-003 proves signing and current code on the phone, WIDGET-004 renders the snapshot, and WIDGET-005 verifies refresh behavior. The planned paid membership remains part of this stage.

WIDGET-001 left one question open. The extension installed, appeared in the gallery, and executed, but two visual changes never appeared on device across several builds and two signers. Retry from a clean test installation before touching widget code. Use the verified backup path before removing any install that contains your ledger.

The widget renders per day and the unknown badge, and swaps to runway when runway drops below days to horizon. It reads the snapshot and never queries the database, because it has no database to query.

Keep the extension excluded from normal builds until this stage resumes. Local app tests do not prove widget rendering or shared-container access; this stage keeps its device checks. App routing, Zustand, NativeWind, and Reanimated remain outside the widget runtime.

## Exit Criteria

- [ ] The paid Apple Developer membership is active.
- [ ] A code change to the widget appears on device, closing the stale-render question from WIDGET-001.
- [ ] The small widget renders per day and the unknown badge from the snapshot.
- [ ] The widget swaps to runway when runway drops below days to horizon.
- [ ] The widget reads only the snapshot and holds no budget arithmetic of its own.
- [x] The stage is split into a device spike, a Feature task, and refresh verification before implementation starts.
